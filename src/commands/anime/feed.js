...
class _Command extends Command {
    constructor (client) {
        super(client, {
            name: "feed",
            description: "Feed your loved ones!",
            usage: "<user|name>",
            guildOnly: false,
            aliases: [],
            permission: {
    ...
    async run(message, args, { GuildDB, prefix, language, translator, responder, rawArgs }) {
        try {
            ...
            const { data } = await axios.get(`https://nekos.life/api/v2/img/feed`).catch(e => {
            ...
            embed.description = translator.translate("FEED_MSG", message.author.mention, (args.join(" ") || "Air"));
            embed.image = {
                url: data.url
            };
