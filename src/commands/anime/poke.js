...
const { Command, Responder } = require("aurora") || global.Aurora;
...
class _Command extends Command {
    ...
    async run(message, args, { GuildDB, prefix, language, translator, responder, rawArgs }) {
        try {
            ...
            const { data } = await axios.get(`https://nekos.life/api/v2/img/poke`).catch(e => {
            ...
            embed.description = translator.translate("POKE_MSG", message.author.mention, (args.join(" ") || "Air"));
            embed.image = {
                url: data.url
            };
