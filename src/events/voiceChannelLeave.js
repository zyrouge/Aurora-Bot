...
module.exports = class {
    ...
    async run(member, oldChannel) {
        if(!member || !oldChannel || !oldChannel.guild) return;
        const GuildDB = await this.client.database.Guild.findOne({ where: { guildID: oldChannel.guild.id } });
        this.Logger(GuildDB, member, oldChannel).catch((e) => {});
    }

    async Logger(GuildDB, member, oldChannel) {
        if(GuildDB && GuildDB.dataValues && GuildDB.dataValues.serverLogs) {
            const LogChannel = oldChannel.guild.channels.get(`${GuildDB.dataValues.serverLogs}`);
